#!/usr/bin/env bash
# Build Marine Console — Beacon (universal). Reads the ESP32 beacon by scanning,
...
cd "$(dirname "$0")"
mkdir -p bin
"$SDK/bin/monkeyc" -f monkey.jungle -d "$DEVICE" -o "bin/MarineBeacon.prg" -y "$KEY"
echo "OK -> bin/MarineBeacon.prg (device=$DEVICE)"
